...
#***SAV
#import sys
# Have removed this as we now use a common yaml configuration file
# Warning: some function still use cfg... so they will fail if ever executed
#sys.path.append("..")
...
def GIOU_xywh_torch(boxes1, boxes2):
    ...
    #print('Inside loss_giou(): ', boxes1.shape, boxes2.shape)
...
def plot_box(bboxes, img, data_id=None, color=None, line_thickness=None):
    # [C,H,W] ---> [H,W,C]
    img = img.permute(0, 2, 3, 1).contiguous()[0].numpy() if isinstance(img, torch.Tensor) else img
    ...
    cv2.imwrite("../data/dataset{}.jpg".format(data_id), img)
