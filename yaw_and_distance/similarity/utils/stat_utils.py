...
def get_knn(index, test_embeddings, k, device, embeddings_come_from_same_source=False):
    test_embeddings = c_f.to_numpy(test_embeddings).astype(np.float32)

    d = test_embeddings.shape[1]
    logging.debug("running k-nn with k=%d" % k)
    logging.debug("embedding dimensionality is %d" % d)
