...
cfg_.fakeMode = std::stoi(info_.hardware_parameters["fake_mode"]);
  ...
  RCLCPP_INFO(rclcpp::get_logger("DiffDriveArduinoHardware"), "init done");
  ...
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  ...
  if (cfg_.fakeMode)
  {
   RCLCPP_INFO(rclcpp::get_logger("DiffDriveArduinoHardware"), "FakeMode Successfully activated!");
   return hardware_interface::CallbackReturn::SUCCESS;
  }
  ...
  const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
{
  ...
  if ((wheel_l_.cmd != 0) || (wheel_r_.cmd != 0)  )
    RCLCPP_INFO(rclcpp::get_logger("write "), "l r %f %f ",wheel_l_.cmd,wheel_r_.cmd);
    //RCLCPP_INFO(rclcpp::get_logger("write "), "rads loop %f %f ",wheel_l_.rads_per_count,cfg_.loop_rate);

  int motor_l_counts_per_loop = wheel_l_.cmd / wheel_l_.rads_per_count / cfg_.loop_rate;
  int motor_r_counts_per_loop = wheel_r_.cmd / wheel_r_.rads_per_count / cfg_.loop_rate;
 
  comms_.set_motor_values(motor_l_counts_per_loop, motor_r_counts_per_loop);
 // RCLCPP_INFO(rclcpp::get_logger("write "), "left %i",motor_l_counts_per_loop);
 // RCLCPP_INFO(rclcpp::get_logger("write "), "right %i",motor_r_counts_per_loop);
  return hardware_interface::return_type::OK;
}
