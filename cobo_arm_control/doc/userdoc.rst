...
The ``path`` attribute of the ``library`` tags refers to the cmake library name of the user defined hardware plugin. See `here <https://github.com/ros-controls/ros2_control_demos/tree/{REPOS_FILE_BRANCH}/example_7/cobo_arm_control.xml>`__ for the complete XML file.
...
The plugin description file is again required for the controller, since it is exported as a library. The controller plugin description file is formatted as follows. See `here <https://github.com/ros-controls/ros2_control_demos/tree/{REPOS_FILE_BRANCH}/example_7/cobo_arm_control.xml>`__ for the complete XML file.
...
.. # Causes the visibility macros to use dllexport rather than dllimport, which is appropriate when building the dll but not consuming it.
.. target_compile_definitions(cobo_controller PRIVATE "CONTROLLER_PLUGIN_DLL")
.. # export plugin
.. pluginlib_export_plugin_description_file(cobo_controller robot_6_dof_controller_plugin_description.xml)
.. # install libraries
.. # ...
...
To view the robot, open a terminal and launch the ``view_cobo.launch.py`` file from the ``cobo_arm_control`` package.

.. code-block:: shell

  ros2 launch cobo_arm_control view_cobo.launch.py
...
Open a terminal and launch the ``cobo_controller.launch.py`` file from the ``cobo_arm_control`` package.

.. code-block:: shell

  ros2 launch cobo_arm_control cobo_controller.launch.py

Finally, open a new  terminal and run the following command.

.. code-block:: shell

  ros2 launch cobo_arm_control send_trajectory.launch.py
