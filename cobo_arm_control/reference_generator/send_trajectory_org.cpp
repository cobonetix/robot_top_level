...
int main(int argc, char ** argv)
{
  ...
  auto pub = node->create_publisher<trajectory_msgs::msg::JointTrajectory>(
    "/right_cobo_controller/joint_trajectory", 10);
  ...
  robot_tree.getChain("base_link", "right_S1_U_A_L2_B1", chain);
  ...
  for (int i = 0; i < trajectory_len; i++)
  {
    // set endpoint twist
    double t = i;
    twist.vel.x(.5 * 0.3 * cos(2 * M_PI * t / trajectory_len));
    twist.vel.y(-0.2 * sin(2 * M_PI * t / trajectory_len));
