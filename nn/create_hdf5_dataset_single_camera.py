"""
Utility script to create HDF5 dataset files for robot control training with single camera.

This script demonstrates how to create properly formatted HDF5 files
that can be used with the RobotDatasetSingleCamera class.
"""
...
def create_hdf5_dataset_single_camera(
    output_path,
    num_samples,
    img_height=224,
    img_width=224,
    ...
    """
    Create an HDF5 dataset file with synthetic robot control data (single camera).
    ...
    print(f"Creating HDF5 dataset with {num_samples} samples (single camera)...")
    ...
    with h5py.File(output_path, 'w') as f:
        ...
        for i in range(0, num_samples, chunk_size):
            ...
            camera[i:end_idx] = np.random.randn(batch_size, 3, img_height, img_width).astype('float32')
...
def main():
    parser = argparse.ArgumentParser(
        description='Create HDF5 datasets for robot control training (single camera)'
    )

    parser.add_argument(
        '--output', '-o',
        type=str,
        default='robot_dataset_single_camera.h5',
        help='Output HDF5 file path (default: robot_dataset_single_camera.h5)'
    )
    ...
    if args.inspect:
        ...
        create_hdf5_dataset_single_camera(
