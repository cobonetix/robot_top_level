...
from robot_control_network_single_camera import RobotControlNetworkSingleCamera


class RobotDatasetSingleCamera(Dataset):
    """
    Dataset for robot control training with single camera.
        ...
        if data_path and os.path.exists(data_path):
            ...
            if file_ext in ['.h5', '.hdf5']:
                ...
                if use_hdf5:
                    ...
                    self.num_samples = len(self.h5_file['camera'])
                    print(f"Loaded {self.num_samples} samples (lazy loading mode)")
                else:
                    # Load entire dataset into memory
                    with h5py.File(data_path, 'r') as f:
                        self.camera_imgs = torch.from_numpy(f['camera'][:]).float()
                    ...
                    print(f"Loaded {len(self.camera_imgs)} samples into memory")

            elif file_ext == '.npz':
                # Load numpy data
                print(f"Loading numpy dataset from {data_path}...")
                data = np.load(data_path)
                self.camera_imgs = torch.from_numpy(data['camera']).float()
                ...
                print(f"Loaded {len(self.camera_imgs)} samples from numpy")
    ...
    def _generate_synthetic_data(self, num_samples):
        ...
        self.camera_imgs = torch.randn(num_samples, 3, self.img_height, self.img_width)
    ...
    def __getitem__(self, idx):
        if self.use_hdf5 and self.h5_file is not None:
            ...
            return {
                'camera': torch.from_numpy(self.h5_file['camera'][idx]).float(),
...
class RobotTrainerSingleCamera:
    """Training manager for the robot control network with single camera."""
...
def main():
    ...
    print("=" * 50)
    print("Robot Control Network Training (Single Camera)")
    print("=" * 50)

    # Create datasets
    print("\nCreating datasets...")
    train_dataset = RobotDatasetSingleCamera(
        num_samples=CONFIG['train_samples'],
        img_height=CONFIG['img_height'],
        img_width=CONFIG['img_width']
    )

    val_dataset = RobotDatasetSingleCamera(
    ...
    # Initialize model
    print("\nInitializing model...")
    model = RobotControlNetworkSingleCamera(
        img_height=CONFIG['img_height'],
        img_width=CONFIG['img_width'],
    ...
    # Initialize trainer
    trainer = RobotTrainerSingleCamera(
        model=model,
