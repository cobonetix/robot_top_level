...
class SimpleLogger:
    """Simple logger that writes to JSON instead of TensorBoard."""
    ...
    def add_epoch(self, epoch_data):
        """Log epoch summary."""
        self.logs['epochs'].append(epoch_data)
...
class IKTrainer:
    """Training manager for the inverse kinematics network (without TensorBoard)."""
    ...
    def __init__(
        ...
        # Simple logger (replaces TensorBoard)
        self.logger = SimpleLogger(log_dir)
        self.logger.add_config({
            'batch_size': batch_size,
    ...
    def train_epoch(self):
        ...
        for batch_idx, batch in enumerate(progress_bar):
            ...
            self.logger.add_scalar('train/batch_loss', loss.item(), global_step)
            for key, value in metrics.items():
                self.logger.add_scalar(f'train/{key}', value, global_step)
    ...
    def train(self, num_epochs, save_freq=10):
        ...
        for _ in range(num_epochs):
            ...
            epoch_data = {
                ...
                'train_position_error': train_pos_error,
                'train_rotation_error': train_rot_error
            }

            if val_loss is not None:
                epoch_data.update({
                    'val_loss': val_loss,
                    'val_position_error': val_pos_error,
                    'val_rotation_error': val_rot_error
                })
                self.scheduler.step(val_loss)

            self.logger.add_epoch(epoch_data)
    ...
    def plot_training_curves(self):
        ...
        # Extract errors from logger
        train_pos_errors = [e['train_position_error'] for e in self.logger.logs['epochs']]
        train_rot_errors = [e['train_rotation_error'] for e in self.logger.logs['epochs']]
        ...
        # Position error plot
        ax2.plot(epochs, train_pos_errors, 'b-', label='Train')
        if self.val_losses and self.val_losses[0] is not None:
            val_pos_errors = [e['val_position_error'] for e in self.logger.logs['epochs']]
            ax2.plot(epochs, val_pos_errors, 'r-', label='Val')
        ...
        # Rotation error plot
        ax3.plot(epochs, train_rot_errors, 'b-', label='Train')
        if self.val_losses and self.val_losses[0] is not None:
            val_rot_errors = [e['val_rotation_error'] for e in self.logger.logs['epochs']]
            ax3.plot(epochs, val_rot_errors, 'r-', label='Val')
        ax3.set_xlabel('Epoch')
        ax3.set_ylabel('Rotation Error (rad)')
...
def main():
    ...
    print("\nLogs are in JSON format. Training curves plot saved to:")
    print(f"  {CONFIG['checkpoint_dir']}/training_curves.png")
